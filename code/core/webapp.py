...
from .database import DatabaseManager, DateORM, EventORM, FamilyORM, PersonORM
...
class GeneWebApp:
    ...
    def __init__(self, db_url: str = "sqlite:///geneweb.db"):
        ...
        self.app = FastAPI(
            title="GeneWeb", description="Modern genealogy application", version="1.0.0"
        )
    ...
    def _setup_routes(self):
        ...
        async def home(request: Request, db: Session = Depends(self.get_db)):
            ...
            recent_persons = (
                db.query(PersonORM).order_by(PersonORM.id.desc()).limit(10).all()
            )
            ...
            return HTMLResponse(
                self.template_env.render_template("home.html", **context)
            )
        ...
        async def persons_list(
            ...
            return HTMLResponse(
                self.template_env.render_template("persons.html", **context)
            )

        @self.app.get("/persons/{person_id}", response_class=HTMLResponse)
        async def person_detail(
            request: Request, person_id: int, db: Session = Depends(self.get_db)
        ):
            """Person detail page."""
            ...
            for family in spouse_families:
                spouse = (
                    family.mother if family.father_id == person_id else family.father
                )
                if spouse:
                    spouses.append(
                        {
            ...
            try:
                consanguinity_result = self.algorithms.calculate_consanguinity_advanced(
                    db, person_id
                )
            except Exception as e:
                # En cas d'erreur (boucles, etc.), on utilise l'algorithme de base
                try:
                    consanguinity_result = self.algorithms.calculate_consanguinity(
                        db, person_id
                    )
                except Exception:
            ...
            return HTMLResponse(
                self.template_env.render_template("person_detail.html", **context)
            )
        ...
        async def person_ancestors(
            ...
            return HTMLResponse(
                self.template_env.render_template("ancestors.html", **context)
            )
        ...
        async def person_descendants(
            ...
            descendants_by_generation = self.algorithms.get_descendants_by_generation(
                db, person_id, generations
            )
            ...
            return HTMLResponse(
                self.template_env.render_template("descendants.html", **context)
            )
        ...
        async def person_relationships(
            ...
            if person.consang > 0:
                try:
                    consanguinity_result = (
                        self.algorithms.calculate_consanguinity_advanced(db, person_id)
                    )
                    consanguinity_info = {
                    ...
                    if consanguinity_result.common_ancestors:
                        ...
                        detailed_paths = []
                        for ancestor_id in list(consanguinity_result.common_ancestors)[
                            :5
                        ]:  # Limit to 5 ancestors
                            paths = self.algorithms.find_detailed_relationship_paths(
                                db, person_id, ancestor_id
                            )
                            if paths:
                                ancestor = (
                                    db.query(PersonORM)
                                    .filter(PersonORM.id == ancestor_id)
                                    .first()
                                )
                                detailed_paths.append(
                                    {"ancestor": ancestor, "paths": paths}
                                )
                        consanguinity_info["detailed_paths"] = detailed_paths

                except Exception as e:
                    print(
                        f"Error calculating consanguinity details for person {person_id}: {e}"
                    )
                    consanguinity_info = None
            ...
            return HTMLResponse(
                self.template_env.render_template("relationships.html", **context)
            )

        @self.app.get(
            "/persons/{person1_id}/relationship_with/{person2_id}",
        ...
        async def relationship_between_persons(
            ...
            relationship_summary = self.algorithms.find_relationship_summary(
                db, person1_id, person2_id
            )
            ...
            return HTMLResponse(
                self.template_env.render_template(
                    "relationship_between.html", **context
                )
            )

        @self.app.get("/families", response_class=HTMLResponse)
        async def families_list(
            request: Request, page: int = 1, db: Session = Depends(self.get_db)
        ):
            """List all families."""
            per_page = 20
            offset = (page - 1) * per_page
            ...
            return HTMLResponse(
                self.template_env.render_template("family_list.html", **context)
            )

        @self.app.get("/search", response_class=HTMLResponse)
        async def search_page(
            request: Request, q: str = "", db: Session = Depends(self.get_db)
        ):
            """Search page."""
            results = []
            ...
            context = {"request": request, "query": q, "results": results}

            return HTMLResponse(
                self.template_env.render_template("search.html", **context)
            )
        ...
        async def statistics(
            ...
            try:
                ...
                persons_with_consanguinity = (
                    db.query(PersonORM).filter(PersonORM.consang > 0).all()
                )
                ...
                consanguinity_stats = {
                    ...
                    "percentage_with_consanguinity": (
                        (len(persons_with_consanguinity) / total_persons * 100)
                        if total_persons > 0
                        else 0
                    ),
                    "high_consanguinity": len(
                        [p for p in persons_with_consanguinity if p.consang >= 0.25]
                    ),
                    "medium_consanguinity": len(
                        [
                            p
                            for p in persons_with_consanguinity
                            if 0.125 <= p.consang < 0.25
                        ]
                    ),
                    "low_consanguinity": len(
                        [p for p in persons_with_consanguinity if 0 < p.consang < 0.125]
                    ),
                    "average_consanguinity": (
                        sum(p.consang for p in persons_with_consanguinity)
                        / len(persons_with_consanguinity)
                        if persons_with_consanguinity
                        else 0
                    ),
                    "max_consanguinity": (
                        max(p.consang for p in persons_with_consanguinity)
                        if persons_with_consanguinity
                        else 0
                    ),
                }
        ...
        async def compute_consanguinity(
            ...
            return RedirectResponse(
                url=f"/statistics?message={message}&success={success}", status_code=303
            )
        ...
        async def import_page(request: Request):
            ...
            return HTMLResponse(
                self.template_env.render_template("import.html", **context)
            )
        ...
        async def admin_page(request: Request, db: Session = Depends(self.get_db)):
            ...
            return HTMLResponse(
                self.template_env.render_template("admin.html", **context)
            )
        ...
        async def compute_consanguinity(
            ...
            """Compute consanguinity for all persons."""
            try:
                results = self.algorithms.compute_all_consanguinity(
                    db, from_scratch=True
                )
        ...
        async def surnames_list(request: Request, db: Session = Depends(self.get_db)):
            ...
            context = {"request": request, "surname_counts": surname_counts}

            return HTMLResponse(
                self.template_env.render_template("surnames.html", **context)
            )

        @self.app.get("/surnames/{surname}", response_class=HTMLResponse)
        async def surname_detail(
            request: Request, surname: str, db: Session = Depends(self.get_db)
        ):
            """Show all persons with a specific surname."""
            persons = db.query(PersonORM).filter(PersonORM.surname == surname).all()

            context = {"request": request, "surname": surname, "persons": persons}

            return HTMLResponse(
                self.template_env.render_template("surname_detail.html", **context)
            )
