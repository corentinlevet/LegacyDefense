...
from sqlalchemy import (
    ...
    ForeignKey, Table, Enum, create_engine, Index
)
...
from .models import Sex, DeathReason, RelationType, Calendar, DatePrecision, EventType, MarriageType, DivorceType
...
family_event_witnesses = Table(
    ...
    Base.metadata,
    Column('family_event_id', Integer, ForeignKey('family_events.id'), primary_key=True),
    Column('person_id', Integer, ForeignKey('persons.id'), primary_key=True)
)
...
class PersonORM(Base):
    ...
    # Relationships
    birth_event = relationship("EventORM", foreign_keys="EventORM.birth_person_id", back_populates="birth_person", uselist=False)
    death_event = relationship("EventORM", foreign_keys="EventORM.death_person_id", back_populates="death_person", uselist=False)
    burial_event = relationship("EventORM", foreign_keys="EventORM.burial_person_id", back_populates="burial_person", uselist=False)
    
    events = relationship("EventORM", foreign_keys="EventORM.person_id", back_populates="person")
    ...
    parent_family = relationship("FamilyORM", foreign_keys=[parent_family_id], back_populates="children")
    
    # Spouse relationships
    families_as_father = relationship("FamilyORM", foreign_keys="FamilyORM.father_id", back_populates="father")
    families_as_mother = relationship("FamilyORM", foreign_keys="FamilyORM.mother_id", back_populates="mother")
    
    # Event witnesses
    witnessed_events = relationship("EventORM", secondary=event_witnesses, back_populates="witnesses")
    witnessed_family_events = relationship("FamilyEventORM", secondary=family_event_witnesses, back_populates="witnesses")
...
class FamilyORM(Base):
    ...
    # Relationships
    father = relationship("PersonORM", foreign_keys=[father_id], back_populates="families_as_father")
    mother = relationship("PersonORM", foreign_keys=[mother_id], back_populates="families_as_mother")
    children = relationship("PersonORM", foreign_keys="PersonORM.parent_family_id", back_populates="parent_family")
...
class EventORM(Base):
    ...
    # Relationships
    person = relationship("PersonORM", foreign_keys=[person_id], back_populates="events")
    birth_person = relationship("PersonORM", foreign_keys=[birth_person_id], back_populates="birth_event")
    death_person = relationship("PersonORM", foreign_keys=[death_person_id], back_populates="death_event")
    burial_person = relationship("PersonORM", foreign_keys=[burial_person_id], back_populates="burial_event")
    
    date = relationship("DateORM")
    place = relationship("PlaceORM")
    witnesses = relationship("PersonORM", secondary=event_witnesses, back_populates="witnessed_events")
...
class FamilyEventORM(Base):
    ...
    date = relationship("DateORM")
    place = relationship("PlaceORM")
    witnesses = relationship("PersonORM", secondary=family_event_witnesses, back_populates="witnessed_family_events")
...
class DatabaseManager:
    ...
    def find_persons_by_name(self, session: Session, first_name: str = None, surname: str = None) -> List[PersonORM]:
    ...
    def get_person_with_family(self, session: Session, person_id: int) -> Optional[PersonORM]:
        """Get person with all family relationships loaded."""
        return session.query(PersonORM).filter(PersonORM.id == person_id).first()
    
    def get_family_with_members(self, session: Session, family_id: int) -> Optional[FamilyORM]:
        """Get family with all members loaded."""
        return session.query(FamilyORM).filter(FamilyORM.id == family_id).first()
    
    def search_persons(self, session: Session, search_term: str, limit: int = 50) -> List[PersonORM]:
        """Search persons by name with fuzzy matching."""
        search_pattern = f"%{search_term}%"
        return session.query(PersonORM).filter(
    ...
    def get_descendants(self, session: Session, person_id: int, max_generations: int = 10) -> List[PersonORM]:
    ...
    def get_ancestors(self, session: Session, person_id: int, max_generations: int = 10) -> List[PersonORM]:
        ...
        for generation in range(max_generations):
            ...
            persons = session.query(PersonORM).filter(
                PersonORM.id.in_(current_generation)
            ).all()
    ...
    def get_statistics(self, session: Session) -> Dict[str, int]:
        ...
        stats['males'] = session.query(PersonORM).filter(PersonORM.sex == Sex.MALE).count()
        stats['females'] = session.query(PersonORM).filter(PersonORM.sex == Sex.FEMALE).count()
        stats['unknown_sex'] = session.query(PersonORM).filter(PersonORM.sex == Sex.NEUTER).count()
        stats['total_events'] = session.query(EventORM).count()
        stats['birth_events'] = session.query(EventORM).filter(EventORM.event_type == EventType.BIRTH).count()
        stats['death_events'] = session.query(EventORM).filter(EventORM.event_type == EventType.DEATH).count()
