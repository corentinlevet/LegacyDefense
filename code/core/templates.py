...
from .models import Person, Family, Date, Place, Sex
...
class TemplateEnvironment:
    ...
    def _register_filters(self):
        ...
        def age_at_death(person: Person) -> Optional[int]:
            ...
            if not birth_date or not death_date or not birth_date.year or not death_date.year:
                return None
            
            age = death_date.year - birth_date.year
            
            # Adjust for month/day if available
            if (birth_date.month and death_date.month and 
                birth_date.month > death_date.month):
                age -= 1
            elif (birth_date.month and death_date.month and 
                  birth_date.month == death_date.month and
                  birth_date.day and death_date.day and
                  birth_date.day > death_date.day):
                age -= 1
        ...
        def living_person(person) -> bool:
            ...
            if hasattr(person, 'birth_event'):
                ...
                if birth_event and hasattr(birth_event, 'date') and birth_event.date and birth_event.date.year:
