...
from .database import DatabaseManager, PersonORM, FamilyORM
from .gedcom import GedcomParser, GedcomExporter
from .models import Person, Family, Database


@click.group()
@click.version_option(version="1.0.0")
@click.option('--verbose', '-v', is_flag=True, help='Enable verbose output')
@click.option('--database-url', '-d', default='sqlite:///geneweb.db', 
...
@click.option('--statistics/--no-statistics', default=True, help='Show import statistics')
@click.pass_context
    ...
    try:
        with db_manager.get_session() as session:
            ...
            if persons:
                ...
                persons_orm = session.query(PersonORM).filter(PersonORM.id.in_(person_ids)).all()
            else:
            ...
            if ctx.obj['verbose']:
                click.echo(f"Found {len(persons_orm)} persons and {len(families_orm)} families")
...
@cli.command()
@click.option('--check/--no-check', default=True, help='Perform database consistency check')
...
def fix_database(ctx, check, fix, backup):
    ...
    try:
        ...
        with db_manager.get_session() as session:
            ...
            orphaned_children = session.query(PersonORM).filter(
                PersonORM.parent_family_id.isnot(None),
                ~PersonORM.parent_family_id.in_(
                    session.query(FamilyORM.id)
                )
            ).all()
            ...
            empty_families = session.query(FamilyORM).filter(
                FamilyORM.father_id.is_(None),
                FamilyORM.mother_id.is_(None)
            ).all()
            ...
            if empty_families:
                ...
                if fix:
                    for family in empty_families:
                        ...
                        children_count = session.query(PersonORM).filter(
            ...
            potential_duplicates = [group for group in duplicate_groups.values() if len(group) > 1]
...
@cli.command()
@click.option('--from-scratch/--incremental', default=False, 
              help='Recompute all consanguinity values from scratch')
@click.option('--verbosity', '-v', type=click.IntRange(0, 2), default=1, 
            ...
            if output_format == 'json':
                ...
                if persons:
                    ...
                    for person in persons:
                        ...
                        click.echo(f"{person.id:<5} {name:<30} {person.sex.value:<5} {occupation:<15}")
                else:
