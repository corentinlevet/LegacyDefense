...
from .models import PersonAPI, FamilyAPI, Sex
...
async def get_persons(
    ...
    for person in persons:
        ...
        if person.birth_event and person.birth_event.date:
            ...
            birth_date = str(person.birth_event.date.year) if person.birth_event.date.year else None
        
        death_date = None
        if person.death_event and person.death_event.date:
            death_date = str(person.death_event.date.year) if person.death_event.date.year else None
...
async def import_gedcom(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error processing GEDCOM: {str(e)}")
    finally:
...
async def get_surnames(
    ...
    surnames = db.query(PersonORM.surname).distinct().filter(
        PersonORM.surname != ""
    ).limit(limit).all()
