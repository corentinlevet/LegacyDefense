...
from .models import (Calendar, Date, DatePrecision, DivorceType, Event,
                     EventType, Family, MarriageType, Name, Person, Place, Sex,
                     Title)
...
class GedcomParser:
    ...
    def parse_file(self, file_path: str) -> Tuple[Dict[int, Person], Dict[int, Family]]:
        ...
        persons = {
            self.xref_to_person_id[xref]: person
            for xref, person in self.persons.items()
        }
        families = {
            self.xref_to_family_id[xref]: family
