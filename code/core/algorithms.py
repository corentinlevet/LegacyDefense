...
from .models import Family, Person, Sex
...
class GenealogyAlgorithms:
    ...
    def get_ancestors(
        self, session, person_id: int, max_generations: int = 50
    ) -> Set[int]:
    ...
    def get_descendants_by_generation(
        self, session, person_id: int, max_generations: int = 50
    ) -> List[List[PersonORM]]:
    ...
    def calculate_relationship_distance(
        self, session, person1_id: int, person2_id: int
        ...
        if person2 and person2.parent_family:
            if (
                person2.parent_family.father_id == person1_id
                or person2.parent_family.mother_id == person1_id
            ):
        ...
        if person1 and person1.parent_family:
            if (
                person1.parent_family.father_id == person2_id
                or person1.parent_family.mother_id == person2_id
            ):
        ...
        if (
            person1
            and person1.parent_family
            and person2
            and person2.parent_family
            and person1.parent_family.id == person2.parent_family.id
        ):
    ...
    def _distance_to_ancestor(
        self, session, person_id: int, ancestor_id: int
    ) -> Optional[int]:
    ...
    def calculate_consanguinity(self, session, person_id: int) -> ConsanguinityResult:
        ...
        common_ancestors = self.find_common_ancestors(
            session, family.father_id, family.mother_id
        )
        ...
        for ancestor_id in common_ancestors:
            father_distance = self._distance_to_ancestor(
                session, family.father_id, ancestor_id
            )
            mother_distance = self._distance_to_ancestor(
                session, family.mother_id, ancestor_id
            )
            ...
            if father_distance is not None and mother_distance is not None:
                ...
                ancestor_consanguinity = self._get_person_consanguinity(
                    session, ancestor_id
                )
    ...
    def compute_all_consanguinity(
        self, session, from_scratch: bool = False, verbosity: int = 1
    ) -> Dict[int, float]:
        """
        ...
        for generation, generation_persons in enumerate(persons_by_generation):
            if verbosity > 1:
                print(
                    f"Processing generation {generation} ({len(generation_persons)} persons)..."
                )
    ...
    def _sort_by_generation(
        self, session, persons: List[PersonORM]
    ) -> List[List[PersonORM]]:
    ...
    def _max_descendant_depth(
        self, session, person_id: int, max_depth: int = 20
    ) -> int:
    ...
    def find_relationship_type(self, session, person1_id: int, person2_id: int) -> str:
        ...
        relationship_info = self.calculate_relationship_distance(
            session, person1_id, person2_id
        )
    ...
    def detect_data_inconsistencies(self, session) -> List[Dict[str, Any]]:
        ...
        for person in persons:
            ...
            if (
                person.birth_event
                and person.death_event
                and person.birth_event.date
                and person.death_event.date
                and person.birth_event.date.year
                and person.death_event.date.year
            ):
        ...
        for family in families:
            if family.father_id and family.mother_id:
                father = (
                    session.query(PersonORM)
                    .filter(PersonORM.id == family.father_id)
                    .first()
                )
                mother = (
                    session.query(PersonORM)
                    .filter(PersonORM.id == family.mother_id)
                    .first()
                )
    ...
    def suggest_potential_matches(
        self, session, person_id: int, threshold: float = 0.8
    ) -> List[Dict[str, Any]]:
        """
                ...
                matches.append(
                    {
                        ...
                        "reasons": self._get_similarity_reasons(
    ...
    def _calculate_person_similarity(
        self, person1: PersonORM, person2: PersonORM
    ) -> float:
    ...
    def _get_similarity_reasons(
        ...
        if person1.first_name and person2.first_name:
            ...
            if name_sim > 0.8:
                reasons.append(
                    f"Similar first names: {person1.first_name} / {person2.first_name}"
                )
        ...
        if person1.surname and person2.surname:
            ...
            if surname_sim > 0.8:
                reasons.append(
                    f"Similar surnames: {person1.surname} / {person2.surname}"
                )
    ...
    def get_statistics(self, session) -> Dict[str, Any]:
        ...
        from .models import EventType, Sex
        ...
        stats["unknown_gender"] = (
            session.query(PersonORM)
            .filter((PersonORM.sex == Sex.NEUTER) | (PersonORM.sex.is_(None)))
            .count()
        )
        ...
        stats["persons_with_birth_date"] = (
            session.query(PersonORM)
            .filter(exists().where(EventORM.birth_person_id == PersonORM.id))
            .count()
        )
        stats["persons_with_death_date"] = (
            session.query(PersonORM)
            .filter(exists().where(EventORM.death_person_id == PersonORM.id))
            .count()
        )
        ...
        try:
            ...
            birth_events = (
                ...
                .filter(
                    EventORM.birth_person_id.isnot(None), EventORM.date_id.isnot(None)
                )
                .all()
            )
        ...
        try:
            ...
            death_events = (
                session.query(EventORM)
                .filter(
                    EventORM.death_person_id.isnot(None), EventORM.date_id.isnot(None)
                )
                .all()
            )
        ...
        families_with_children = (
            session.query(FamilyORM)
            .join(PersonORM, PersonORM.parent_family_id == FamilyORM.id)
            .distinct()
            .count()
        )
        ...
        if stats["total_families"] > 0:
            total_children = (
                session.query(PersonORM)
                .filter(PersonORM.parent_family_id.isnot(None))
                .count()
            )
        ...
        if stats["total_families"] > 0:
            family_sizes = (
                session.query(
                    FamilyORM.id, func.count(PersonORM.id).label("child_count")
                )
                .outerjoin(PersonORM, PersonORM.parent_family_id == FamilyORM.id)
                .group_by(FamilyORM.id)
                .all()
            )
        ...
        surname_counts = (
            session.query(PersonORM.surname, func.count(PersonORM.id).label("count"))
            .filter(PersonORM.surname.isnot(None), PersonORM.surname != "")
            .group_by(PersonORM.surname)
            .order_by(func.count(PersonORM.id).desc())
            .limit(20)
            .all()
        )

        stats["most_common_surnames"] = [
            (surname, count) for surname, count in surname_counts
        ]
        ...
        try:
            ...
            roots = (
                session.query(PersonORM)
                .filter(PersonORM.parent_family_id.is_(None))
                .all()
            )
        ...
        stats["living_persons"] = (
            stats["total_persons"] - stats["persons_with_death_date"]
        )

        # Data quality metrics - using EXISTS for birth events with dates and places
        stats["persons_with_complete_birth_info"] = (
            session.query(PersonORM)
            .filter(
                exists().where(
        ...
        stats["persons_with_occupation"] = (
            session.query(PersonORM)
            .filter(PersonORM.occupation.isnot(None), PersonORM.occupation != "")
            .count()
        )
    ...
    def _calculate_max_depth(self, session, person_id: int, current_depth: int) -> int:
        ...
        children = (
            ...
            .filter(PersonORM.parent_family_id == FamilyORM.id)
            .all()
        )
        ...
        for child in children:
            child_depth = self._calculate_max_depth(
                session, child.id, current_depth + 1
            )
    ...
    def calculate_consanguinity_advanced(
        self, session, person_id: int
    ) -> ConsanguinityResult:
        """
        ...
        if self._person_has_loop(session, person_id, set()):
            raise ValueError(
                f"Boucle détectée dans l'arbre généalogique pour la personne {person_id}"
            )
        ...
        for ancestor_id in common_ancestors:
            ...
            if father_distance is not None and mother_distance is not None:
                ...
                if ancestor_id not in self._consanguinity_cache:
                    ...
                    ancestor_result = self.calculate_consanguinity_advanced(
                        session, ancestor_id
                    )
    ...
    def _calculate_kinship_coefficient(
        self, session, person1_id: int, person2_id: int
    ) -> float:
        """
        ...
        for ancestor_id in common_ancestors:
            ...
            if dist1 is not None and dist2 is not None:
                ...
                ancestor_consanguinity = self._get_consanguinity_recursive(
                    session, ancestor_id
                )
    ...
    def _has_loop_from_person(self, session, person_id: int, visited: Set[int]) -> bool:
        ...
        if person.parent_family.father_id:
            has_loop |= self._has_loop_from_person(
                session, person.parent_family.father_id, visited.copy()
            )

        if person.parent_family.mother_id and not has_loop:
            has_loop |= self._has_loop_from_person(
                session, person.parent_family.mother_id, visited.copy()
            )
    ...
    def compute_all_consanguinity_advanced(
        self, session, from_scratch: bool = False, verbosity: int = 1
    ) -> Dict[int, float]:
        """
        ...
        for i, person in enumerate(sorted_persons):
            ...
            if verbosity > 1 and i % 100 == 0:
                print(
                    f"  Progression: {i}/{total_persons} ({i/total_persons*100:.1f}%)"
                )
    ...
    def find_detailed_relationship_paths(
        self, session, person1_id: int, person2_id: int
        ...
        for ancestor_id in common_ancestors:
            ...
            if path1 and path2:
                ...
                relationship_name = self._generate_relationship_name(
                    len(path1), len(path2)
                )
    ...
    def _determine_path_type(
        self, path1: List[FamilyLink], path2: List[FamilyLink]
    ) -> str:
    ...
    def find_relationship_summary(
        self, session, person1_id: int, person2_id: int
    ) -> Optional[RelationshipSummary]:
        """
        ...
        primary_relationship = self._classify_relationship(
            primary_path.person1_distance, primary_path.person2_distance
        )
    ...
    def get_person_family_context(self, session, person_id: int) -> Dict[str, Any]:
        ...
        children = (
            session.query(PersonORM)
            .filter(
                PersonORM.parent_family_id.in_(
        ...
        for child in children:
            child_families = (
                session.query(FamilyORM)
                .filter(
                    (FamilyORM.father_id == child.id)
                    | (FamilyORM.mother_id == child.id)
                )
                .all()
            )
